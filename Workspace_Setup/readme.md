## Initial Google Workspace Setup Stage

💡Domain Business Name and Workspace setup/Registration


![Workspace_Setup](Create-1.JPG)


![Workspace_Setup](btworx-1.JPG)

## Insert website called BTWORX.NET - purchase new domain from Squarespace

![Workspace_Setup](btworx-2.JPG)

![Workspace_Setup](custom-domain-1.JPG)

![Workspace_Setup](btworx-5-new-user.JPG)

![Workspace_Setup](btworx-6.JPG)


## Review Google Workspace Administrator Dashboard 

![Workspace_Setup](btworx-7-dashboard.JPG)

## Review Google Workspace Administrator Dashboard apps to roll out

![Workspace_Setup](btworx-7.JPG)



